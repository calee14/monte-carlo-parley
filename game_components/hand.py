...
class Hand:
    ...
    def determine_hand_rank(self):
        ...
        sorted_cards = sorted(self.m_cards, key=lambda card: card.rank.value, reverse=True)
        ...
        if four_kind:
            ...
            for card in sorted_cards:
                if card.rank.value != self.top_value:
                    self.secondary_value = card.rank.value
                    break
            return
        # full house
        ...
        if three_kind and pairs:
            ...
            self.secondary_value = max(pairs)
            return
            ...
            self.secondary_value = three_kind[1]
            return
        # flush
        flush_suit = None
        ...
        if three_kind:
            ...
            self.secondary_value = kickers[0] if kickers else 0
            return
        # Check for two pair
        if len(pairs) >= 2:
            ...
            self.secondary_value = pairs[1]
            # Find the highest kicker
            for card in sorted_cards:
                if card.rank.value != self.top_value and card.rank.value != self.secondary_value:
        ...
        self.secondary_value = sorted_cards[1].rank.value if len(sorted_cards) > 1 else 0
    ...
    def _is_royal_flush(self, sorted_cards, suit_counter):
        ...
        for suit, count in suit_counter.items():
            if count >= 5:
                ...
                royal_values = {CardValue.ACE.value, CardValue.KING.value, 
                              CardValue.QUEEN.value, CardValue.JACK.value, CardValue.TEN.value}
    ...
    def _is_straight_flush(self, sorted_cards, suit_counter):
        ...
        for suit, count in suit_counter.items():
            if count >= 5:
                suit_cards = sorted([card for card in sorted_cards if card.suit == suit], 
                                   key=lambda card: card.rank.value, reverse=True)
    ...
    def _is_straight(self, sorted_cards):
        ...
        if (CardValue.ACE.value in unique_values and 
            CardValue.FIVE.value in unique_values and 
            CardValue.FOUR.value in unique_values and 
            CardValue.THREE.value in unique_values and 
            CardValue.TWO.value in unique_values):
